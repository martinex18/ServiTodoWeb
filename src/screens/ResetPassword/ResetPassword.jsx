                            ...
                            {!emailSent && (
                                <div className="mt-8 space-y-2">
                                    ...
                                    <div className="relative">
                                        ...
                                        <input type="email" name="email" placeholder="ejemplo@correo.com" value={email} onChange={(e) => setEmail(e.target.value)} className={inputClass} />
                                    </div>
                                </div>
                            )}

                            {error && (
                                <p className="text-sm text-error bg-error/10 px-3 py-2 rounded-md mt-6">{error}</p>
                            )}

                            {emailSent && (
                                <p className="text-sm text-success bg-success/10 px-3 py-4 rounded-md mt-6">
                                    Se ha enviado un correo electrónico para restablecer la contraseña. Por favor, revisa tu bandeja de entrada o spam.
                                </p>
                            )}

                            <div className="mt-8 space-y-3">
                                {!emailSent && (
                                    <button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 transition text-white font-semibold py-3 rounded-lg cursor-pointer" disabled={loading}>
                                        {loading ? "Enviando..." : "Enviar"}
                                    </button>
                                )}
                                <button className="w-full text-gray-600 hover:text-blue-600 transition font-medium cursor-pointer" onClick={() => navigate('/login')}>
                                    Volver al inicio
                                </button>
                            </div>
                        </form>
